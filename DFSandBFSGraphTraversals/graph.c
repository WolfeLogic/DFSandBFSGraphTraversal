/*
 * CS 261-400 F2016 Data Structure
 * Assignment 7
 * Name: Drew Wolfe
 * Date: 11/30/2016
 */
...
{
    assert(graph != NULL);
    Deque* stack = malloc(sizeof (Deque)); //create a deque to be used as a stack

    clearVisited(graph); //reset the visited property of each vertex
    dequeNew(stack); //initialize the deque
    dequePushFront(stack, source); //add the source node to the stack
    
    while (!dequeIsEmpty(stack)) //as long as there are verticies in the deque
    {
        //pop a vertex off the stack and mark it as visited
        Vertex* curVert = dequeFront(stack);
        curVert->isVisited = 1;
        dequePopFront(stack);

        //test each neighbor of the current vertex, checking to see whether or not they have already been visited
        for (int i = 0; i < curVert->numNeighbors; i++)
        {
            //push a previously unvisited vertex on the stack
            if (!curVert->neighbors[i]->isVisited)
            {
                dequePushFront(stack, curVert->neighbors[i]);
            }
        }

        //return true if the current vertex is also the destination
    ...
    free(stack); //free the stack | deallocates the memory previously allocated by the call to malloc

    return 0; //return false if none of the verticies traversed were the destination vertex
}
...
{
    assert(graph != NULL);
    Deque* queue = malloc(sizeof(Deque)); //create a deque to be used as a queue

    clearVisited(graph); //reset the visited property of each vertex
    dequeNew(queue); //initialize the deque
    dequePushBack(queue, source); //add the source node to the queue

    while (!dequeIsEmpty(queue))  //as long as there are verticies in the deque
    {
        //remove a vertex from the front of the queue and mark it as visited
        Vertex* curVert = dequeFront(queue);
        curVert->isVisited = 1;
        dequePopFront(queue);
        ...
        for (int i = 0; i < curVert->numNeighbors; i++)
        {
            //add a previously unvisited vertex to the back of the queue
            if (!curVert->neighbors[i]->isVisited)
            {
                dequePushBack(queue, curVert->neighbors[i]);
            }
        }
